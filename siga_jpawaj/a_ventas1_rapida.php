  ...
  <tr>
    <td colspan="3" bgcolor="#00238c" class="tit_menu_sup"><table width="831" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="487" align="center" bgcolor="#00238C" class="TITULO"><span class="TITULO_AMARILLO">VENTAS MOSTRADOR - (RÁPIDA)</span></td>
...
$result=mysqli_query($connec,"select * from a_provisional  where codcli_provi=$xcodigo_cli  AND tmov_provi='I' order by id");
}
	
$totalpp=mysqli_num_rows($result);
...
while ($tabla=mysqli_fetch_array($result)) {
		...
		$id=$tabla["id"];
		$codcli_provi=$tabla["codcli_provi"];
		$nrodocu_provi=$tabla["nrodocu_provi"];
		$fecmov_provi=$tabla["fecmov_provi"];
		$coditem_provi=$tabla["coditem_provi"];
		$desc_provi=$tabla["desc_provi"];
		$cant_provi=$tabla["cant_provi"];
		$tmov_provi=$tabla["tmov_provi"];
		$pvent_provi=$tabla["pvent_provi"];
		$total_provi=$cant_ppend*$pvent_ppend;		
		$pcosto_provi=$tabla["pcosto_provi"];	
		$codven_provi=$tabla["codven_provi"];
...
<form id="form1" name="form1" method="post" onSubmit="return checkSubmit();" action="a_ventas2_rapida.php" onkeypress="javascript:if(event.keyCode==13){return false;}" >
